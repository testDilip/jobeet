...
class lime_output
{
  ...
  public function echoln($message, $colorizer_parameter = null, $colorize = true)
  {
    if ($colorize)
    {
      $message = preg_replace_callback('/(?:^|\.)((?:not ok|dubious|errors) *\d*)\b/', function ($matches) {return $this->colorizer->colorize($matches[0], 'ERROR');}, $message);
      //$message = preg_replace('/(?:^|\.)((?:not ok|dubious|errors) *\d*)\b/e', '$this->colorizer->colorize(\'$1\', \'ERROR\')', $message);
      $message = preg_replace_callback('/(?:^|\.)(ok *\d*)\b/', function ($matches) {return $this->colorizer->colorize($matches[0], 'INFO');}, $message);
      //$message = preg_replace('/(?:^|\.)(ok *\d*)\b/e', '$this->colorizer->colorize(\'$1\', \'INFO\')', $message);
      $message = preg_replace_callback('/"(.+?)"/', function ($matches) {return $this->colorizer->colorize($matches[0], 'PARAMETER');}, $message);
      //$message = preg_replace('/"(.+?)"/e', '$this->colorizer->colorize(\'$1\', \'PARAMETER\')', $message);
      $message = preg_replace_callback('/(\->|\:\:)?([a-zA-Z0-9_]+?)\(\)/', function ($matches) {return $this->colorizer->colorize($matches[0].$matches[1].'()', 'PARAMETER');}, $message);
